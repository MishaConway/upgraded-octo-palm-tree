    ...
    while ( directoryEntry )
    {
        if( directoryEntry->d_type != DT_DIR )
        {
            if (strstr(directoryEntry->d_name, fext.c_str())){
                std::string currentFile = std::string( directoryEntry->d_name);
                if( return_absolute_paths )
                    currentFile = cleaned_path + currentFile;
                files.push_back( currentFile );
            }
...
std::string File::ReadAllTextWithIncludePreprocessing( const std::string& path ){
    std::string text = File::ReadAllText(path);
    auto include_directive_matches = GetRegexMatches( text, "#include([^\n]+)" );
    for( unsigned int i = 0; i < include_directive_matches.size(); i++ )
    {
        std::string include_file = StripString( include_directive_matches[i], "\"" );
        include_file = File::DirName(path) + ExplodeString( include_file, " \t" ).back();
        ...
        if( include_source.empty() )
            ...
            text = StringReplace( text, include_directive_matches[i], include_source );
    }
    
    return text;
}

std::string File::DirName( const std::string& filepath ){
    size_t found = filepath.find_last_of("/\\");
    return filepath.substr(0,found) + "/";
}

std::string File::PathJoin( const std::string& a, const std::string& b){
    return StringReplace( a + "/" + b, "//", "/");
}
